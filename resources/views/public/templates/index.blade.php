<nav class="navbar navbar-expand-lg navbar-dark bg-primary fixed-top shadow">
    <div class="container">
        <a class="navbar-brand" href="{{ route('/') }}">
            <img src="{{ asset('assets/images/' . config('app.icon')) }}" alt="Logo {{ config('app.icon') }}" width="30" height="30" class="d-inline-block align-text-top">
            {{ config('app.name') }}
        </a>
            ...
            <div class="offcanvas-body">
                <ul class="navbar-nav m-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href=" {{ $navMenuParent->url }}" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            {{ $navMenuParent->name }}
                        </a>
                ...
                <form action="{{ route('/') }}" method="get" class="d-flex" role="search">
                    <div class="input-group">
                        ...
                        <input type="text" name="search" id="search" value="{{ request('search') }}" class="form-control" placeholder="disini...">
                    </div>
                </form>
            </div>
        </div>
...
<section class="wrapper pt-5">
    <div id="carouselControls1" class="carousel slide mt-4 pt-3" data-bs-ride="carousel">
        <div class="carousel-indicators">
            ...
            @endphp
            <button type="button" data-bs-target="#carouselControls1" data-bs-slide-to="{{ $no++ }}" class="{{ $class }}" aria-current="true" aria-label="slide {{ $loop->iteration }}"></button>
            @endforeach
        </div>
        <div class="carousel-inner rounded-0">
            ...
            <div class="carousel-item carousel-item-overlay <?= $class ?>" data-bs-interval="5000">
                ...
                @endphp
                <img src="{{ $url }}" class="d-block w-100" alt="{{ $url }}" aria-label="slide {{ $no++ }}">
            ...
            <div class="col-12 col-md-7">
                <div id="carouselControls2" class="carousel slide" data-bs-ride="carousel">
                    <div class="carousel-indicators">
                        ...
                        $no == 0 ? ($class = 'active') : ($class = '');
                        @endphp
                        <button type="button" data-bs-target="#carouselControls2" data-bs-slide-to="{{ $no++ }}" class="{{ $class }}" aria-current="true" aria-label="slide {{ $loop->iteration }}"></button>
                        @endforeach
                    </div>
