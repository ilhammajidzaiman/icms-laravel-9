<x-app-layout>
    ...
    <x-slot name="button">
        ...
        @else
            <x-link href="{{ route($page->route . '.index') }}" value="{{ __('kembali') }}" icon="bi bi-arrow-left"
                class="rounded-pill btn btn-secondary" />
        @endif --}}
    </x-slot>
    ...
    <form action="{{ route($page->route . '.index') }}" method="get" class="mb-3">
        <div class="d-flex flex-wrap justify-content-end">
            ...
            <div class="mb-3 mx-1 ">
                <button class="rounded-pill btn btn-primary px-3" type="submit">
        ...
        <x-table class="mb-3">
            <x-table.thead>
                <x-table.tr>
                    ...
                    <x-table.th>{{ __('judul') }}</x-table.th>
                    <x-table.th>{{ __('penulis') }}</x-table.th>
            ...
            <x-table.tbody>
                @foreach ($record as $key => $item)
                    <x-table.tr>
                        ...
                        <x-table.td>{{ $item->favicon }}</x-table.td>
                        <x-table.td>{{ $item->logo }}</x-table.td>
                        <x-table.td>
                            {{-- @if ($page->routeName !== 'trash')
                                <x-link.button href="{{ route($page->route . '.show', ['id' => $item->uuid]) }}"
                                    value="{{ __('lihat') }}" icon="bi bi-eye" class="btn-outline-primary btn-sm" />
                                <x-link.button href="{{ route($page->route . '.edit', ['id' => $item->uuid]) }}"
                                    value="{{ __('ubah') }}" icon="bi bi-pencil-square"
                                    class="btn-outline-success btn-sm" />
                                <x-link.button href="{{ route($page->route . '.delete', ['id' => $item->uuid]) }}"
                                    value="{{ __('hapus') }}" icon="bi bi-trash2" data-confirm-delete="true"
                                    class="btn-outline-danger btn-sm" />
                            @else
                                <x-link.button href="{{ route($page->route . '.show', ['id' => $item->uuid]) }}"
                                    value="{{ __('lihat') }}" icon="bi bi-eye" class="btn-outline-primary btn-sm" />
                                <x-link.button href="{{ route($page->route . '.restore', ['id' => $item->uuid]) }}"
                                    value="{{ __('pulihkan') }}" icon="bi bi-arrow-clockwise"
                                    class="btn-outline-success btn-sm" />
                                <x-link.button href="{{ route($page->route . '.forceDelete', ['id' => $item->uuid]) }}"
                                    value="{{ __('hapus') }}" icon="bi bi-trash2" data-confirm-delete="true"
                                    class="btn-outline-danger btn-sm" />
                            @endif --}}
                        </x-table.td>
