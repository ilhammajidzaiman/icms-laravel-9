        ...
        <div class="col-12 col-md">
            <div class="card">
                <div class="card-content">
                    <div class="card-body">
                        <form action="{{ route(Request::segment(1) . '.nav-menu.child.update', $navMenuChild->uuid) }}"
                            method="post">
                            @method('put')
                            @csrf
                            <div class="row">
                            ...
                            <div class="row">
                                <x-form-input-row type="number" name="order" label="urutan"
                                    value="{{ old('order', $navMenuChild->order) }}" class="col-md-2" />
                            </div>
                            <div class="row">
                                <x-form-input-row type="text" name="name" label="nama"
                                    value="{{ old('name', $navMenuChild->name) }}" class="col-md-4" />
                            </div>
                            <div class="row">
                                <x-form-input-row type="text" name="url" label="url"
                                    value="{{ old('url', $navMenuChild->url) }}" class="col" />
                            </div>
